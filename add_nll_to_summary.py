...
def compute_nll_metrics(p: Path) -> Tuple[Optional[float], Optional[float], int]:
    ...
    if isinstance(obj, dict):
        ...
        if vals and all(isinstance(v, dict) for v in vals) and any(('avg_nll' in v or 'total_nll' in v or 'total_tokens' in v) for v in vals):
            entries = vals
...
def find_experiments_jsons(nll_runs_dir: Path) -> List[Path]:
    if not nll_runs_dir.exists():
        return []
    return sorted([p for p in nll_runs_dir.iterdir() if p.is_file() and p.name.startswith('experiments') and p.suffix == '.json'])
...
def main(root: str, out_name: Optional[str] = None) -> None:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='在 music quality summary CSV 中加入来自 nll_runs/experiments*.json 的 nll 指标')
    parser.add_argument('folder', help='包含 summary CSV 和 nll_runs 子目录的根目录')
