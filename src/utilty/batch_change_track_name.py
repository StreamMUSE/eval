...
def main():
    parser = argparse.ArgumentParser(
        description="在每个 generated 文件夹上分两步替换 track 名称。"
    )
    parser.add_argument(
        "root",
        type=Path,
        help="根目录，递归查找名为 generated 的文件夹（例：realtime/baseline）",
    )
    parser.add_argument(
        "--change-script",
        type=Path,
        default=Path("src/utilty/change_track_name.py"),
        help="change_track_name.py 路径",
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="仅打印将要执行的命令，不真正执行"
    )
    ...
    for g in gen_dirs:
        ...
        rc = run_rename(
            script, g, "User Melody", "Guitar", out_dir=step1, dry=args.dry_run
        )
        if rc != 0:
